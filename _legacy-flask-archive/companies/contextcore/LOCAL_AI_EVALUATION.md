...
## Part 4 — ContextCore verdict (tested against this app's actual prompts)

Ran Ollama (`llama3.2:3b`, already pulled) against ContextCore's real
`EXTRACTION_PROMPT`, `CONFLICT_PROMPT`, and `generate_answer()` prompt shape
— using real text from the seeded `distribution-intelligence` collection,
not synthetic examples. Read-only test; nothing in `app.py` was changed.

| Task | Real prompt used | Result |
|---|---|---|
| Entity/relationship extraction | `EXTRACTION_PROMPT` on the full "Horror and the October Corridor" doc | Valid JSON, but **factually wrong**: it emitted a relationship claiming *A Quiet Place* broke the horror opening-weekend record — the source text says that about *It*. Also emitted a few relationship edges referencing strings not present in its own `entities` list (e.g. `"Andy Muschietti's Halloween"`), which `app.py`'s existing name-matching dedup would silently drop rather than corrupt the graph — but the *It*/*A Quiet Place* mix-up would go straight in as a confident, wrong fact. |
| Conflict detection | `CONFLICT_PROMPT`, once with a planted real contradiction, once with two genuine non-conflicting passages | Correct both times (`conflict: true` / `conflict: false` with sane explanations). |
| RAG generation | Same shape as `generate_answer()`'s prompt, real retrieved chunks | Reasonable prose, followed the `[Chunk N]` citation format, stayed grounded in the provided context. |

**Verdict: (a) — an API key is still needed for acceptable quality**,
specifically for extraction. ContextCore's actual value is a knowledge
graph you can trust to be grounded in the source text, not just
well-formed JSON — a model that confidently swaps which film broke which
record would silently poison that graph with false claims presented as
true ones, which is worse than the current honest "extraction skipped, no
API key" state. Conflict detection and generation held up in this small
test and could be worth revisiting locally later (they're both
lower-complexity, more constrained tasks than open-ended extraction), but
nothing has been wired in — `app.py`'s Claude integration is unchanged.
