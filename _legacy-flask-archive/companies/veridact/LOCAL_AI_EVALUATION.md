...
> **Note for Veridact specifically**: this company's own README states it
> "never analyses pixels to decide whether something looks AI" — its entire
> product premise is cryptographic proof (ECDSA signatures, C2PA manifests),
> not inference. Adding local-model integration here sits against that
> documented design. This doc is still being dropped in per the portfolio-wide
> instruction, but the recommendation is: **don't wire this in.** If you
> genuinely see a fit that doesn't compromise the proof-not-inference
> positioning, make the case explicitly in your report rather than adding it
> quietly.
